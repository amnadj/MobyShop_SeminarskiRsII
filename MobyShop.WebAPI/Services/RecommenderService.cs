    ...
    public class RecommenderService : IRecommender
    {
        ...
        public List<Artikli> GetSlicneArtikle(int artikalID)
        {
            ...
            foreach (var item in proizvodi)
            {
                ...
                if (slicnosti > 0.99)
                {
                    Database.Artikli element1 = _context.Artikli.Include(y => y.Proizvodjaci).Include(z => z.Modeli).Where(x => x.ArtikalId == item.Key).FirstOrDefault();
        ...
        {
            List<Database.Artikli> aktivniProizvodi = _context.Artikli.Include(y => y.Proizvodjaci).Include(z => z.Modeli).Where(x => x.ArtikalId != artikalId).ToList();
